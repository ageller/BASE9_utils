...
Another interactive tool is available, though not shows in the example .ipynb file: `cluster.createInteractiveSelector`, which can be used to interactively select regions in kinematic and distance space that may define the cluster and see those selected stars on a CMD.  This may be useful for a cluster that is heavily embedded in a rich field where the automated algorithms in our code have difficulty selecting members.  Using `cluster.createInteractiveSelector` can provide helpful limits to trim down the data before sending through `cluster.runAll` for example.  (This interactive is also built in `Bokeh` and should be executed in a similar manner as `cluster.createInteractiveIsochrone`.)
